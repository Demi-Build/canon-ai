...
# ---------------------------------------------------------------------------
# Loadout mode — full per-class generation (stats fixed to budget + spells/abilities)
# ---------------------------------------------------------------------------


class TestClassPhaseLoadoutMode:
    """ClassPhase(loadout_specs=...) generates complete classes: stats fixed to
    the spec budget within role ranges, plus skeleton-rolled spells/abilities
    whose names come from the LLM."""

    @staticmethod
    def _warrior_loadout():
        from canon.pipeline.phases.class_archetype import ClassLoadoutSpec, LoadoutGroup
        from canon.skeleton.core import SkeletonField, SkeletonSpec

        abil_spec = SkeletonSpec(
            entity_type="ability",
            fields={
                "stat": SkeletonField(choices=[("STR", 1)]),
        ...
        return ClassLoadoutSpec(
            ...
            stat_budget=95,
            role_ranges={"primary": (14, 18), "secondary": (12, 16), "dump": (8, 12)},
            stat_names=["STR", "DEX", "CON", "INT", "WIS", "CHA", "LUCK"],
            starting_weapon="Iron Sword",
            abilities=LoadoutGroup(
                skeleton_spec=abil_spec,
                starting=[{"stat": "STR", "stamina_cost": 8}] * 2,
                pool=[{"stat": "STR", "stamina_cost": 4}] * 2,
                prompt_kwargs={"archetype": "warrior"},
            ),
        )

    def test_loadout_generates_complete_class(self):
        flavor = json.dumps({"name": "Ironbreaker", "description": "A bulwark.", "lore": "Old guard."})
        abilities = json.dumps([{"name": f"Maneuver {i}", "description": "x"} for i in range(4)])
        ctx = make_ctx([flavor, abilities])
        ClassPhase(loadout_specs=[self._warrior_loadout()]).run(ctx)

        arch = ctx.bible.class_archetypes["warrior"]
        assert arch.name == "Ironbreaker"
        assert arch.archetype == "warrior"
        assert arch.starting_weapon == "Iron Sword"

    def test_loadout_stats_sum_to_budget_and_keep_emphasis(self):
        flavor = json.dumps({"name": "Ironbreaker"})
        abilities = json.dumps([{"name": f"A{i}"} for i in range(4)])
        ctx = make_ctx([flavor, abilities])
        ClassPhase(loadout_specs=[self._warrior_loadout()]).run(ctx)

        stats = ctx.bible.class_archetypes["warrior"].stat_template
        assert sum(stats.values()) == 95          # fixed to budget (was 80 template)
        assert stats["STR"] == max(stats.values())  # warrior STR emphasis preserved
        assert 14 <= stats["STR"] <= 18

    def test_loadout_warrior_has_abilities_no_spells(self):
        flavor = json.dumps({"name": "Ironbreaker"})
        abilities = json.dumps([{"name": f"A{i}"} for i in range(4)])
        ctx = make_ctx([flavor, abilities])
        ClassPhase(loadout_specs=[self._warrior_loadout()]).run(ctx)
        ...
        arch = ctx.bible.class_archetypes["warrior"]
        assert len(arch.abilities) == 2        # starting
        assert len(arch.ability_pool) == 2     # level-up pool
        assert len(arch.spells) == 0           # warriors use quest abilities, not spells
