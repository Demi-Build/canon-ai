...
def make_responder(num_maps: int = 3) -> Any:
    ...
    def respond(request: Any) -> str:  # noqa: PLR0911 (branchy by necessity)
        ...
        # ------------------------------------------------------------------
        # Event  — event_generation prompt
        # ------------------------------------------------------------------
        if "event" in msg and ("encounter" in msg or "occurrence" in msg or "challenge" in msg):
            n = counters["event"]
            counters["event"] += 1
            # Try to detect puzzle from context, default to combat
            event_type = "puzzle" if "puzzle" in msg else "combat"
...
class TestMazeworldFullPipeline:
    ...
    def test_events_json_entry_type_field(self, pipeline_run):
        ...
        for i, evt in enumerate(events):
            ...
            assert evt["type"] in ("combat", "puzzle"), (
                f"events[{i}] type {evt['type']!r} not in (combat, puzzle)"
            )
    ...
    def test_events_json_puzzle_has_choices(self, pipeline_run):
        tmp = pipeline_run["tmp"]
        events = _load(tmp / "events/events.json")
        puzzle_events = [e for e in events if e.get("type") == "puzzle"]
        for evt in puzzle_events:
            assert "choices" in evt, (
                f"puzzle event {evt.get('id')} missing 'choices'"
            )
