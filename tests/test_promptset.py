...
class TestPromptSetABC:
    def test_abstract_method_count(self) -> None:
        """PromptSet declares exactly 33 abstract methods (17 original + 16 v0.2)."""
        abstract_methods = PromptSet.__abstractmethods__
        assert len(abstract_methods) == 33, (
            f"Expected 33, got {len(abstract_methods)}: {sorted(abstract_methods)}"
        )
    ...
    def test_expected_method_names(self) -> None:
        ...
        expected = {
            ...
            "spell_pool_generation",
            "spell_pool_generation_with_feedback",
            "ability_generation",
...
class TestMaxTokens:
    ...
    def test_max_tokens_reasonable_ceiling(
        ...
        """No method should request more than 2000 tokens.

        Ceiling raised from 1024 → 2000: story_generation (2000) and
        dialogue_tree_generation (1500) were deliberately enlarged to stop real
        Claude responses truncating mid-JSON (factions/beats, multi-node trees).
        """
        for req in _all_prompts(ps, simple_character, simple_bible):
            assert req.max_tokens <= 2000, (
                f"max_tokens={req.max_tokens} exceeds 2000 for system={req.system[:60]!r}"
            )
