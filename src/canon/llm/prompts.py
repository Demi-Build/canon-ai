...
class PromptSet(ABC):
    ...
    def story_generation(
        self, seed: str, structure: dict | None = None
    ) -> LLMRequest: ...
...
_JSON_ONLY = "Respond with valid JSON only — no prose before or after the object."
...
class DefaultPromptSet(PromptSet):
    ...
    def story_generation(
        ...
        return LLMRequest(
            ...
            user_message=(
                ...
                '"escalation_arc": ["<beat 1>", "<beat 2>", "<beat 3>"], '
    ...
    def story_generation_with_feedback(
        ...
        base = self.story_generation(seed, structure)
        return LLMRequest(
            system=base.system,
    ...
    def item_generation(
        ...
        if item_category == "weapon":
            shape = (
                ...
                '"damage_type": "<damage type>", '
                '"weapon_category": "<light|medium|heavy>", '
    ...
    def event_generation_with_feedback(
        ...
        base = self.event_generation(context, skeleton, event_type)
        return LLMRequest(
            system=base.system,
            user_message=base.user_message + _feedback_section(feedback),
            examples=base.examples,
