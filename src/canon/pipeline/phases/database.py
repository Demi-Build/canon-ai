...
class DatabasePhase:
    ...
    def _generate_one(
        ...
        *,
        ...
        # Roll skeleton (empty dict when no spec)
        skeleton = (
            roll_skeleton(spec.skeleton_spec, ctx.rng)
        ...
        try:
            ...
            llm_response = {"name": f"{spec.entity_type} {index}"}
        ...
        if spec.cross_room_dedup:
            ...
            if dup_fields:
                ...
                try:
                    retry_response = json.loads(retry_result)
                except json.JSONDecodeError:
                    retry_response = llm_response
