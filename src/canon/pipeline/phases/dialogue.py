...
# TODO(v0.3): Replace `quest_bearing_roles` + `_generate_quest_variants` with a general
# `DialogueVariantSpec(suffix, context_tags, hint)` system and `specs_by_role` mapping.
# Enables time/season/event/relationship axes for any game genre (Harvest Moon, etc.).
# Trees generated sequentially, each seeing prior entry-node prompts as context.
# `DialogueTree` gains `context_tags: dict[str,str]`; game runtime owns tag resolution.
# See memory: project_dialogue_variant_system.md
"""
...
class DialoguePhase:
    ...
    def generate_for_character(
        ...
        quest_variants: bool = False,
    ) -> None:
        """Generate dialogue tree(s) for a single character.

        Public entrypoint for callers that source characters outside
        ``bible.characters`` — e.g. database-style entities adapted into
        transient ``Character`` objects.  Stores the tree(s) in
        ``ctx.bible.dialogues`` and sets the character's ``dialogue_tree_*_id``
        fields, exactly as ``run()`` does for in-bible characters.

        When ``quest_variants`` is True, generates the 4-variant set
        (base + complete/failed/incomplete); otherwise a single base tree.
        """
