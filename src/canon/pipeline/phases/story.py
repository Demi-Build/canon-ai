    ...
    def run(self, ctx: Any) -> None:
        ...
        def generate(feedback: list[str] | None = None) -> str:
            ...
            if feedback:
                ...
                request = ctx.prompts.story_generation(
                    seed=seed, structure=self.story_structure
                )
            # Store last prompt/response on the function object so the outer
            # scope can read them after retry_with_feedback returns. A nonlocal
            # variable can't be captured here because generate is a nested
            # function (not a closure over a mutable cell that retry_with_feedback
            # can see). Using function attributes avoids that limitation cleanly.
        ...
        def validate(content: str) -> tuple[bool, list[str]]:
            ...
            if not isinstance(data, dict):
                return False, ["Response root must be a JSON object."]
