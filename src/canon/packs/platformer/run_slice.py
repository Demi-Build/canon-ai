...
The runner ships inside the package (row P0-4): ``python -m
canon.packs.platformer.run_slice`` from any cwd, installed wheel or checkout.
``canon world new`` spawns exactly this module; ``examples/run_platformer_slice.py``
is a shim onto ``main`` so the documented commands keep working.

    # deterministic, $0
    uv run --extra platformer python -m canon.packs.platformer.run_slice \
        --backend fake --output-dir /tmp/plat_slice --seed emberfall_001

    # real content
    ANTHROPIC_API_KEY=sk-... uv run --extra platformer --extra anthropic \
        python -m canon.packs.platformer.run_slice --backend anthropic \
...
Then review PNGs under <output-dir>/review/ and play a level:
    ...
    uv run --extra platformer --extra play \
        python -m canon.packs.platformer.play <output-dir> l1
"""
...
def _run_schedulers(
    ...
    *,
    ...
    """Run the slice under the scheduler ``args`` picked (``--orchestrate``
    → the DAG, else the sequential pipeline). Split out of ``main`` so the
    cancel handling (row P1-A4.5) wraps ONE call; behavior is unchanged."""
    if args.orchestrate:
...
def main(argv: list[str] | None = None) -> int:
    """Entry point — ``argv`` defaults to ``sys.argv[1:]`` (argparse); returns
    the process exit code so ``python -m`` and the ``examples/`` shim share it."""
    ...
    parser.add_argument(
        ...
        "pixel art). Examples proving the swap (package data beside "
        "graphics.py): graphics_specs/{snes_pixel,rendered_hd}.json.",
    )
    parser.add_argument(
        "--models", default=None,
    ...
    # The registry id rides the context so SliceManifestPhase mirrors it into
    # manifest.json.pack_type (P0 paper P.4.1). Function-level import: spec.py
    # builds its seed FROM this pack's op tables.
    ...
    if args.art_sample:
        from canon.packs.platformer.art_sampling import run_art_sample
    ...
    if args.art_lock:
        from canon.packs.platformer.art_lock import load_art_lock
    ...
    # the scheduler's own ``run_end`` carries ``cancelled: true`` + ``kept``,
    # and this runner exits ``EXIT_CANCELLED`` (3) — nothing landed is undone.
    from canon.pipeline.steplog import EXIT_CANCELLED, RunCancelled

    try:
        _run_schedulers(args, ctx, output_dir, rules=rules, tiles=tiles, variants=variants,
                        image_producer=image_producer, graphics=graphics,
                        music_producer=music_producer, sfx_producer=sfx_producer,
                        combat=combat, vlm_judge=vlm_judge)
    except RunCancelled as cancelled:
        print(f"\n!! Cancelled before {cancelled.node} · {cancelled.item} — "
              f"{len(cancelled.kept)} completed item(s) kept (see {output_dir}/.canon/log.jsonl)")
        return EXIT_CANCELLED
    if ctx.steplog is not None and ctx.steplog.cancelled is not None:
        # The orchestrated scheduler escalates the node that raised and carries
        # on with independent work until each hits its own boundary; the log
        # remembers the cancel, so the exit code is honest either way.
    ...
    print(
        ...
        f"python -m canon.packs.platformer.play {output_dir} l1"
    )
    return 0
