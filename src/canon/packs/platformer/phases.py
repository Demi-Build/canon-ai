...
from canon.pipeline.steplog import step as step
...
#: Row P0-10 (master §3.0-E): the ONE ``node_item`` emitter + its A4.5 cancel
#: boundary now lives in ``canon.pipeline.steplog`` so the dungeon's item
#: loops go through the SAME function (a dungeon create is cancellable for
#: free). Re-exported here verbatim: every ``from canon.packs.platformer.phases
#: import step`` call site is unchanged.
...
class StagePhase:
    ...
    def run(self, ctx: Any) -> None:
        from canon.packs.platformer.effects import sanitize_effects
