...
from canon.packs.platformer.layers import BackgroundPhase, TileAssignmentPhase
...
from canon.packs.platformer.render import RenderPhase
...
def _run_warnings(ctx: Any) -> list[str]:
    ...
    from canon.packs.platformer.vlm_qa import derive_qa_warnings, qa_report_rel
...
class SliceManifestPhase:
    ...
    #: This writer's own pack id — the fallback for ``manifest.json.pack_type``
    #: when the context carries none (legacy callers). P0 paper P.4.1: the
    #: mirror is written on EVERY manifest write, so a stamp survives resume.
    ...
    def run(self, ctx: Any) -> None:
        ...
        manifest = {
            # FIRST key by contract (P.4.1 mirror rule; the one sanctioned
            # additive delta to the platformer tree, P.9 R14).
