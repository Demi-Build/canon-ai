...
Ships INSIDE the wheel (2026-09-01, extending the P0-4 promotion that moved
``run_slice`` in the same way) so a bundled cradle can ▶ Play a level before
W2.0 — ``python -m canon.packs.platformer.play`` from any cwd, installed
wheel or checkout; ``examples/`` keeps a ``platformer_play.py`` shim onto ``main``.
pygame is imported lazily inside the entry points, never at module import,
so ``import canon.packs.platformer`` stays pygame-free.

    uv run --extra platformer --extra play \
        python -m canon.packs.platformer.play <data_dir> [level_id]
...
composes with PLAT_CAPTURE for a headless contact strip:
    ...
    PLAT_ANIM=enemy:ember_hopper PLAT_CAPTURE=shots \
        python -m canon.packs.platformer.play <data_dir>
...
# module keeps ONE evaluator rather than a second copy here (the pack ships
# inside the installed package since row P0-4 — no sys.path fix-up needed).
from canon.packs.platformer.animation_spec import eval_ladder as _eval_ladder  # noqa: E402
from canon.packs.platformer.animation_spec import load_spec as _load_anim_spec  # noqa: E402
...
def _parse_spawn(raw: str) -> tuple[int, int] | None:
    """``"x,y"`` → ``(x, y)`` cell coordinates for ``PLAT_SPAWN``; empty →
    ``None``; anything else is reported on stderr and ignored (a bad hook
    value must never stop the harness from opening)."""
    raw = raw.strip()
    if not raw:
        return None
    try:
        ...
        print(f"PLAT_SPAWN: expected 'x,y' cells, got {raw!r} — using the level's spawn", file=sys.stderr)
        return None
...
class _Hooks:
    ...
    def __init__(self) -> None:
        ...
        # PLAT_SPAWN="x,y" — start the player at this cell instead of the
        # level's spawn point (row P1-A4.5's ``sandbox_level(level_id?,
        # spawn?)``: drop into the sandbox room next to the thing being
        # tuned). Cells, not pixels; malformed = ignored loudly on stderr.
        # W2.0's session runtime keeps this hook as its spawn seam.
        self.spawn: tuple[int, int] | None = _parse_spawn(os.environ.get("PLAT_SPAWN", ""))
...
def main(argv: list[str] | None = None) -> int:
    """Entry point: ``python -m canon.packs.platformer.play <data_dir> [level_id]``.

    Wraps the harness's entry logic verbatim (the in-package move, extending
    the P0-4 ``run_slice.main`` shape so the ``examples/`` shim and cradle's
    ``play_level`` spawn the same thing). ``argv`` defaults to
    ``sys.argv[1:]``; every ``PLAT_*`` env hook is still read where it was
    (``_Hooks`` / module load), and the exit codes are unchanged: no args →
    usage on stderr, exit 1; pygame missing → the same install hint, exit 1;
    a completed session → 0. ``-h`` / ``--help`` prints the usage and exits 0
    (the only addition — it never reaches the pygame import).
    """
    ...
    level_id = args[1] if len(args) > 1 else "l1"
    ...
    try:
        ...
        raise SystemExit(
            ...
            "--extra play python -m canon.packs.platformer.play ..."
        ) from None
...
def run_level(
    ...
    # defaults mirror src/canon/packs/platformer/combat.py.
    ...
    if hooks.spawn is not None and not in_room:
        # PLAT_SPAWN overrides the level's spawn for the level the harness
        # opened with (rooms keep their own entry cells), clamped to the map.
        spawn = {
            "x": max(0, min(int(hooks.spawn[0]), int(width) - 1)),
            "y": max(0, min(int(hooks.spawn[1]), int(height) - 1)),
        }
