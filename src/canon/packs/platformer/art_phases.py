...
from canon.packs.platformer.style import background_role
...
def _add_image_cost(ctx: Any, producer: Any) -> None:
    """Drain PRODUCER's metered image spend into ``ctx.stats.image_cost_usd``
    (row P1-A6). The exact twin of ``audio_phases._add_audio_cost`` for the
    image lane, which had no accumulator at all: ``_cost_block`` read
    ``stats.image_cost_usd`` and every sprite/backdrop/animation event
    journalled a confident ``costCents: 0`` no matter what fal or PixelLab
    billed — P.8.2's never-a-silent-$0 rule, broken on the product's most
    expensive leg. The producer meters each backend call itself (retries
    included); this reads and RESETS that meter, so a producer shared by
    several phases bills each phase for its own calls and never twice for one.
    No-op without stats or without a producer (the placeholder path is a real
    $0)."""
    stats = getattr(ctx, "stats", None)
    if stats is None or producer is None:
        return
    drain = getattr(producer, "drain", None)
        ...
        for stage_id, tileset in ctx.bible.tilesets.items():
            ...
            for slot in tileset.slots:
                ...
                if slot.params.get("water_deep"):
                    ...
                    from canon.packs.platformer.tileset import (
                        derive_water_deep,
                    )
...
class SpriteAnimationPhase:
    ...
    def _animate_one(
        ...
        from canon.packs.platformer.vlm_qa import author_animation_spec
    ...
    def _sheet_frames(
        ...
        def _edit(note: str = "") -> Any:
            ...
            # Row P1-A6: the img2img leg is a BILLED call the producer's own
            # generate choke point never sees (the phase calls `edit_backend`
            # directly, and it may be a different backend — ticket 7). Meter it
            # here, right after the call, and keep the prompt that was really
            # sent so the animation event carries P.8.3's `gen.prompt_hash`.
            meter = getattr(self.producer, "meter", None)
            if callable(meter):
                meter(edit_backend)
            self.producer.last_prompt = prompt + note
    ...
    def _animate_actor(
        ...
        from canon.packs.platformer.vlm_qa import STATE_LOOP_MODES
    ...
    def _write_actor_frames(
        ...
        their final ``size``."""
        # Function-local like every other vlm_qa import in this module:
        # vlm_qa imports art_phases at load time, so a top-level import
        # would be circular. Renormalize passes ``loop`` through from disk,
        # so a manifest without it lands on this fallback.
        from canon.packs.platformer.vlm_qa import STATE_LOOP_MODES
