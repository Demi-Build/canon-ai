...
def art_provenance(
    *,
    ...
    from canon.packs.platformer.phases import PROMPT_VERSION
...
class DiffusionSheetProducer:
    ...
    def __init__(self, backend: Any, edit_backend: Any = None) -> None:
        ...
        #: Row P1-A6: the running image spend of every backend call this
        #: producer has made and the phases have not yet folded into
        #: ``ctx.stats.image_cost_usd``. The audio lane already meters this way
        #: (``audio_phases._add_audio_cost``); the image lane did not, so the
        #: journal's most expensive rows read a confident $0. Accumulated HERE
        #: (the one choke point every generate passes through) rather than at
        #: the phase's call site, because one ``tile_image``/``sprite_image``
    ...
    def drain(self) -> float:
        ...
        last drain. The art phases drain into ``ctx.stats.image_cost_usd`` at
        the end of each phase, so a producer shared by several phases bills
    ...
    def _generate(
        ...
        try:
            out = self.backend.generate(prompt, width, height)
            self.meter()
            return out
        except Exception as e:  # noqa: BLE001 — provider-specific types
            # No meter on this path: backends stamp ``last_cost`` only AFTER a
            # call returns, so a raise leaves the PREVIOUS call's figure there
            # and metering it would bill the same generation twice.
            ...
            self.last_prompt = sanitized
            out = self.backend.generate(sanitized, width, height)
            self.meter()
            return out
